...
def execute_query(query, params=None, fetch=True):
    ...
    if any(token in query for token in ("{", "}")) and not query.strip().startswith(
        "CALL"
    ):
        raise ValueError(
    ...
    for i in percents:
        if not (i + 1 < len(query) and query[i + 1] in ("s", "%")):
            raise ValueError(f"Potential unsafe percent-formatting in query: {query}")
    conn = get_connection()
    try:
        with conn.cursor() as cursor:
    ...
    try:
        with conn.cursor() as cursor:
            for query, params in queries_and_params:
                ...
                ) and not query.strip().startswith("CALL"):
                    raise ValueError(
                        f"Unsafe query detected: braces/bracket formatting found in query.\n"
                        f"Use parameterized queries with %s placeholders only, never f-strings or .format().\nQuery: {query}"
                    )
