...
def upload_profile_picture_url(current_user):
    try:
        ...
        AVATARS_DIR = (
            "avatars_whitelist"  # Directory with allowed images (could point elsewhere)
        )
        allowed_filenames = [
            f
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "image_index must be an integer"}
                ),
                400,
            )
        if image_index < 0 or image_index >= len(allowed_filenames):
            return (
                jsonify({"status": "error", "message": "image_index is out of range"}),
                400,
            )
...
def api_v1_forgot_password():
    try:
        ...
        user = execute_query("SELECT id FROM users WHERE username = %s", (username,))
